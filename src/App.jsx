...
function RoleCell({ value, onOpen, finde, compact, editable, esInicio, conflicto }) {
  ...
  return <td className={`border-b border-r border-slate-200 p-0 text-center font-semibold ${codigoCls(v, finde)} ${esInicio ? "ring-2 ring-inset ring-emerald-700" : ""} ${conflicto ? "ring-4 ring-inset ring-red-600" : ""}`}><button type="button" onClick={onOpen} disabled={!editable} className={`relative w-full ${compact ? "h-9 text-[11px]" : "h-11 text-[12px]"} font-semibold tracking-wide outline-none transition ${editable ? "cursor-pointer hover:brightness-95 focus:ring-2 focus:ring-emerald-700" : "cursor-default"}`} title={conflicto ? "Advertencia: tiene actividad programada aunque el rol no está en turno" : editable ? "Cambiar marca del día" : "Active edición del funcionario para modificar"}><span className="inline-flex min-w-8 items-center justify-center rounded-md bg-white/45 px-1.5 py-0.5 shadow-sm ring-1 ring-black/5">{v || "—"}</span>{conflicto && <span className="absolute right-0 top-0 flex h-4 w-4 items-center justify-center rounded-bl-md bg-red-700 text-[10px] text-white">!</span>}{esInicio && <span className="absolute bottom-0 left-1 right-1 rounded-t bg-emerald-900 px-1 text-[8px] font-bold tracking-wider text-white">INICIO</span>}</button></td>;
}
