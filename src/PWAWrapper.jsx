...
function InstallBanner({ onInstall, onDismiss }) {
  return (
    <div
      className="pnlq-no-print fixed bottom-[calc(5rem+env(safe-area-inset-bottom))] left-1/2 z-50 w-[calc(100%-2rem)] max-w-md -translate-x-1/2 rounded-2xl border border-emerald-200 bg-white p-4 shadow-2xl ring-1 ring-emerald-100 lg:bottom-4"
      role="alertdialog"
...
function OfflineBanner({ lastLoadedAt }) {
  return (
    <div
      className="pnlq-no-print fixed bottom-[calc(5rem+env(safe-area-inset-bottom))] left-1/2 z-50 -translate-x-1/2 rounded-2xl border border-amber-300 bg-amber-50 px-5 py-3 shadow-xl ring-1 ring-amber-200 lg:bottom-4"
      role="alert"
    >
...
function UpdateBanner({ onUpdate, onDismiss, urgent = false, remoteVersion }) {
  ...
  return (
    <div
      className={`pnlq-no-print fixed bottom-[calc(5rem+env(safe-area-inset-bottom))] left-1/2 z-50 w-[calc(100%-2rem)] max-w-md -translate-x-1/2 rounded-2xl border p-4 shadow-2xl lg:bottom-4 ${
